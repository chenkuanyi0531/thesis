...
input_testing = '../src/list/ucf_val.list'; 
test = list(open(input_testing, "r"))

len_window = 64
fps = 32

annots = loadmat('ground_truth_ucf.mat')
val_annot = [item.flat[0] for item in annots['tempo']['data']]
data_annot = [item.flat[0] for item in annots['tempo']['textdata']]
...
for i in test:
    ...
    normal = True if 'Normal' in vid else False

    annotation_name = os.path.join(vidname_[len(vidname_)-2], vidname_[len(vidname_)-1] + '.mp4')

    if not normal:
        for j in range(len(data_annot[0])):
            temp = data_annot[0][j]
            if annotation_name == temp[0].astype(str):
                val = val_annot[0][j]
                break
    ...
    gtt = np.zeros(int(tmp.split(' ')[1]))
    
    gtt[int(val[1]):int(val[2])] = 1
    if int(val[3]) > 0:
        gtt[int(val[3]):int(val[4])] = 1
    ...
    for k in range(int(tmp.split(' ')[1])):
        ...
        if k % fps == 0:
            ...
            plt.subplot(2,1,1)
            img_file = os.path.join(args.rgb, vidname_[len(vidname_)-2], vidname, 'i', str(k+1).zfill(6) + '.jpg')
